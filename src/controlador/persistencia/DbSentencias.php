...
interface DbSentencias {
    ...
    //Horario Suplente
    const LISTAR_HORARIOS_SUPLENTES="SELECT horario_suplente.*,nombre_carrera,resolucion,nombre_materia,nombre_sede,numero_sede,nombre_curso,titular.nombre_profesor AS nombre_titular,titular.apellido_profesor AS apellido_titular,suplente.nombre_profesor AS nombre_suplente,suplente.apellido_profesor AS apellido_suplentefrom horario_suplente INNER JOIN materia ON (fk_materia=id_materia) INNER JOIN plan_de_estudios ON (fk_plan_de_estudio=id_plan) INNER JOIN carrera ON (fk_carrera=id_carrera) INNER JOIN curso ON (fk_curso=id_curso) INNER JOIN sede ON (horario_suplente.fk_sede=id_sede)INNER JOIN profesor AS titular ON (fk_titular=titular.`id_profesor`)INNER JOIN profesor AS suplente ON (fk_suplente=suplente.`id_profesor`)WHERE estado_horario_suplente=1";
    ...
    const BUSCAR_TITULARES="SELECT DISTINCT (horarios.fk_profesor) FROM horarios INNER JOIN profesor ON (fk_profesor=id_profesor) WHERE horarios.ciclo_lectivo_horario=? AND horarios.estado=1";
    const BUSCAR_SEDES="SELECT DISTINCT (id_sede),nombre_sede,numero_sede FROM horarios INNER JOIN curso ON (fk_curso=id_curso)INNER JOIN sede ON (fk_sede=id_sede)WHERE horarios.ciclo_lectivo_horario=?AND horarios.estado=1";
    const BUSCAR_CURSOS="SELECT DISTINCT (id_curso),nombre_curso FROM horarios INNER JOIN curso ON (fk_curso=id_curso) WHERE horarios.ciclo_lectivo_horario=? AND horarios.estado=1";
    ...
    const BUSCAR_MATERIAS_HORARIOS_SUPLENTES="SELECT DISTINCT (id_materia),nombre_materia FROM horarios INNER JOIN materia ON (fk_materia=id_materia) WHERE horarios.fk_plan=? AND horarios.estado=1";
    ...
    const VERIFICAR_HORARIO_SUPLENTE="SELECT EXISTS(SELECT horario_suplente.* FROM horario_suplente WHERE fk_sede=? AND fk_ciclo_lectivo=? AND fk_curso=? AND fecha_inicio=? AND fecha_fin=? AND estado_horario_suplente=1) AS existe";
